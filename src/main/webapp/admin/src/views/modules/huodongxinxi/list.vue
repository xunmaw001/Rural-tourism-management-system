		    ...
		    label="咨询电话">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('huodongxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('huodongxinxi','活动报名') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="huodongbaomingCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'活动报名':'' }}</el-button>
                <el-button v-if="isAuth('huodongxinxi','活动报名') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="huodongbaomingCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'活动报名':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('huodongxinxi','活动报名') && contents.tableBtnIcon == 0" type="success" size="mini" @click="huodongbaomingCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'活动报名':'' }}</el-button>
